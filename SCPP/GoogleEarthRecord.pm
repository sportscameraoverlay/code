...
sub recordTourGE($$$$){
    ...
    while(!$exit_now){
        ...
        if(($ge_status > 0) and !$ge_thr->is_running()){
            ...
            #Recording should finish before GE unless debug is high
            if(($record_status != 2) and ($debug <= 2)){
                print "GE thread ended abnormally\n";
                $exit_err = 1;
                $exit_now = 1;
            }
...
sub recordControl($$$){
    ...
    $capture_offset[0] = int(($xvfb_res[0] - $capture_res[0]) / 2);
    $capture_offset[1] = int(($xvfb_res[1] - $capture_res[1]) / 3); #Divide by 3 so to "look forward"
