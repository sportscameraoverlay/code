...
sub genKML($$$$){
    ...
    if($map_file){
        ...
        foreach my $skirun (keys %skiruns){
            ...
            $document->appendChild($placemark_SR);
            ...
            foreach my $node_id (sort {$a <=> $b} keys $skiruns{$skirun}{'nodes'}){
                $coordinates_SR->appendTextNode("$skiruns{$skirun}{'nodes'}{$node_id}{'lon'},$skiruns{$skirun}{'nodes'}{$node_id}{'lon'},0\n");
            }
        }
    }
...
sub parseMapData($){
    ...
    foreach my $way ($xmldoc->findnodes('/osm/way')){
        ...
        foreach my $nd ($way->findnodes('nd')){
            ...
            ${$skiruns_ref}{$way_id}{'nodes'}{$node_ref}{'lat'} = $nodes{$node_ref}{'lat'};
            ${$skiruns_ref}{$way_id}{'nodes'}{$node_ref}{'lon'} = $nodes{$node_ref}{'lon'};
        }
    }
