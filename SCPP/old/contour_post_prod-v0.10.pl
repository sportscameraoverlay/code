...
readSubs();
mkdir $tmpdir;
my $bim = overlay1(3008, 156.3, 182.56, 1, 1);
createImage($bim, 0, 0, 1);
exit;
...
sub generateOverlays(){
    ...
    foreach my $time (sort keys %GPS_data){
        ...
        #Create the correct overlay
        my $bim = overlay1($altitude, $speed, $maxspeed, $time, 1);

        #Now create the image in the correct res and correct positioning
        createImage($bim, 0, 0, $image_num);
        $image_num++;
...
##############################################################################
#CreateImage - create an overlay image in the right res for the input video
#The following is passed in
#1)Reference to the overlay image
#2)x positioning of overlay image on final image 
#3)y positioning of overlay image on final image
#4)Image number
##############################################################################
sub createImage($$$$){

    (my $bim, my $x_pos, my $y_pos, my $num) = @_;

    my $res=1.5;#$orig_vid_res[1] / 720; #All dimensions are based off 720p res
    ...
    $im->copy(${$bim},$x_pos,$y_pos,0,0,${$bim}->width,${$bim}->height);
...
sub overlay1($$$$$){
    ...
    my $text1 = $bim->colorAllocate(0,210,0);
    my $text2 = $bim->colorAllocate(0,0,0);

    #TODO This should be done in the base image (static)
    #Print headings 
    #$image->stringFT($fgcolor,$fontname,$ptsize,$angle,$x,$y,$string)
    $bim->stringFT($text2,$ubuntufont,32,0,70,70,"Altitude");
    $bim->stringFT($text2,$ubuntufont,32,0,350,70,"Max Speed");
    $bim->stringFT($text2,$ubuntufont,32,0,650,70,"Current Speed");
    #Print Units
    $bim->stringFT($text2,$ubuntuCfont,40,0,$altTextboxOff + $altTextboxLen,$textHeight,"m");
    $bim->stringFT($text2,$ubuntuCfont,36,0,$mspeedFTextboxOff + $mspeedFTextboxLen,$textHeight,"km/h");
    $bim->stringFT($text2,$ubuntuCfont,36,0,$speedFTextboxOff + $speedFTextboxLen,$textHeight,"km/h");

    #Split the speed vars into intergers and fractions
    my $maxspeedInt = int($maxspeed);
    my $speedInt = int($speed);
    my $maxspeedF = '.' . (sprintf( '%.1f', ($maxspeed - $maxspeedInt)) * 10);
    my $speedF = '.' . (sprintf( '%.1f', ($speed - $speedInt)) * 10);
    print "speed: $speed\n";
    print "maxspeed: $maxspeed\n";
    print "speed Fraction: $speedF\n";
    print "maxspeed Fraction: $maxspeedF\n";
    print "speed int: $speedInt\n";
    print "maxspeed int: $maxspeedInt\n";
    ...
    printString($text1,$ubuntufont,$text_sizeM,$mspeedFTextboxOff,$textHeight,$maxspeedF,"Max_Speed",$mspeedFTextboxLen,$epoch_time,0,\$bim) if($frac);
    printString($text1,$ubuntufont,$text_sizeL,$speedTextboxOff,$textHeight,$speedInt,"Speed",$speedTextboxLen,$epoch_time,1,\$bim);
    printString($text1,$ubuntufont,$text_sizeM,$speedFTextboxOff,$textHeight,$speedF,"Speed_Fraction",$speedFTextboxLen,$epoch_time,0,\$bim) if($frac);
