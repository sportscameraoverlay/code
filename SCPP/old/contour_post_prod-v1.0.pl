...
# 1.00 PJ 15/02/13 First Release that does pretty much what I want! (Bug fixes from v0.12)
#
###############################################################################
...
my $images_per_sec = 5; #this needs to be divisable wholey into the vid_out_framerate!
...
sub generateOverlays(){
    ...
    my $num_of_images = $subtitle_length * $GPS_period * $images_per_sec; #Number of images for progess display
    ...
    foreach my $time (sort keys %GPS_data){
        ...
        for( my $i = 0; $i < $num_interpolated_pts; $i++){
            ...
            #Update maxpeed if the current speed is larger
            $maxspeed = $current_speed if( $current_speed > $maxspeed );
            ...
            positionImage($bim, 0, 100, 75, $image_num);
            $image_num++;
            my $tmp = ($speed + ($i * $speed_inc));
            print "Image $image_num, Speed: $tmp\n" if(defined($speed_inc) and $debug);
        }
...
sub overlay2($$$$){
    ...
    #Allocate some colours (R,G,B)
    my $needle1 = $bim->colorAllocate(255,0,0);
    my $needle2 = $bim->colorAllocate(0,0,255);
    my $black = $bim->colorAllocate(0,0,0);
    my $text1 = $bim->colorAllocate(255,255,255);

    my $deg_per_speed = ($speedo_max_angle - $speedo_min_angle) / $speedo_max_value;
    my $speed_ang = (($speed * $deg_per_speed) + $speedo_min_angle);
    my $mspeed_ang = (($maxspeed * $deg_per_speed) + $speedo_min_angle);
    ...
    $altitude = sprintf( '%04.0f', $altitude);
