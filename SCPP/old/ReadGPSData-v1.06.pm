...
use SCPP::Common;
use SCPP::Config qw(:debug :tmp :video :directionsmooth);
...
my $earths_radius = 6378000; #in metres
...
sub GPSPointsCalc($){
    ...
    foreach my $GPSline (sort {$a <=> $b} keys %{$GPS_data_ref}){
        ...
        print "Direction we are heading at GPS line # $GPSline is $direction degrees (using $count pts)\n" if($debug > 2);
