...
my $baseImageRes=720;
my $vid_out_framerate = 50; 
my $vid_out_file = "out.mp4";
...
##############################################################################
#Subroutine to blend images with video using melt
##############################################################################
#melt -track /home/paul/contour_auth/100MEDIA/FILE0034.MOV -track contour_img-%01d.png ttl=50 -transition composite geometry=0%,0%:100%x100%:100
#melt -track /home/paul/contour_auth/100MEDIA/FILE0034.MOV in=0 out=500 -track /home/paul/contour_auth/100MEDIA/FILE0007.MOV in=0 out=500 \
#-track /home/paul/contour_auth/100MEDIA/snowboard3.png in=0 out=500 -track contour_img-%01d.png ttl=50 in=0 out=500 \
#-transition composite: a_track=0 b_track=1 geometry=75%,75%:25%x25%:100 -transition composite: a_track=0 b_track=2 geometry=0%,0%:100%x100%:100 \
#-transition composite: a_track=0 b_track=3 geometry=0%,0%:100%x100%:100 
sub meltImages(){

    my $frames_per_image = $vid_out_framerate / $GPS_period;
    my $orig_vid_frames = int($orig_vid_framerate * $video_length); 
    my $vid_out_frames = $vid_out_framerate * $subtitle_length;
    print "Input Video Frames: $orig_vid_frames\n" if($debug);
    print "GPS Period: $GPS_period\n" if($debug);
    print "Output Video Frames: $vid_out_frames\n" if($debug);
    print "Frames per image: $frames_per_image\n" if($debug);

    my $command = "melt -track $video_file in=0 out=$orig_vid_frames -track $base_image_file in=0 out=$vid_out_frames -track $tmpdir/contour_img-%d.png ttl=$frames_per_image in=0 out=$vid_out_frames -transition composite: a_track=0 b_track=1 geometry=0%,0%:100%x100%:100 -transition composite: a_track=0 b_track=2 geometry=0%,0%:100%x100%:100 -consumer avformat:$vid_out_file vcodec=$vid_out_codec vpre=$vid_out_quality r=$vid_out_framerate acodec=$audio_out_codec ab=$audio_out_bitrate";

    print "Melt Command:\n$command\n" if($debug);
    system($command) and die "Failed to create output video: $!";
}
...
#2)Input video framerate
#3)Output video filename
#4)Output video framerate
##############################################################################
sub convertFramerate($$$$) {

    my ($in_filename, $in_framerate, $out_filename, $out_framerate) = @_;

    #Check if we actuatly need to do anything
    if($in_framerate == $out_framerate) {
        return 2;
    }
...
sub runMelt($$){
    ...
    exec("$command -progress 2>&1 >$melt_out_tmp") or die "failed to run $command";


    
    #TODO: I need to tail the log file and work out when to continue, while printing out progress...
    #Then I can use this!!
...
sub createImage($$$$$){
    ...
    warn "Altitude string $altitude is too long! Actual: $textLen[2] pixels, Max: $altTextboxLen*$res pixels.\n" if($textLen[2] > $altTextboxLen*$res);
    my $altOffset = $altTextboxLen*$res - $textLen[2] + $altTextboxOff*$res;

    @textLen = GD::Image->stringFT(0,$ubuntufont,60*$res,0,0,0,$speed);
    warn "Speed string $speed is too long! Actual: $textLen[2] pixels, Max: $speedTextboxLen*$res pixels.\n" if($textLen[2] > $speedTextboxLen*$res);
    my $speedOffset = $speedTextboxLen*$res - $textLen[2] + $speedTextboxOff*$res;

    @textLen = GD::Image->stringFT(0,$ubuntufont,60*$res,0,0,0,$maxspeed);
    warn "Max Speed string $maxspeed is too long! Actual: $textLen[2] pixels, Max: $mspeedTextboxLen*$res pixels.\n" if($textLen[2] > $mspeedTextboxLen*$res);
    my $mspeedOffset = $mspeedTextboxLen*$res - $textLen[2] + $mspeedTextboxOff*$res;

    #Print the strings
    $im->stringFT($text1,$ubuntufont,60*$res,0,$altOffset*$res,$textHeight*$res,$altitude);
    $im->stringFT($text1,$ubuntufont,60*$res,0,$mspeedOffset*$res,$textHeight*$res,$maxspeed);
    $im->stringFT($text1,$ubuntufont,60*$res,0,$speedOffset*$res,$textHeight*$res,$speed);
...
sub createSubs(){
    ...
    foreach my $line (@output) {
        ...
        if( $line =~ /Stream #0.* Video: .+, .+, (\d+)x(\d+).*, (\d+) kb\/s, (\d+(.\d+)?) fps,/){
            @orig_vid_res = ($1,$2);
            $orig_vid_bitrate = $3;
            $orig_vid_framerate = $4;
        }
    }
	progress($process_name, 100);
}
