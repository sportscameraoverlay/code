...
#Image stabilization
my $stabilize_video = 0; #0 = no image stabilization, 1 = stabilize image
...
checkGPSData();
genKML(\%GPS_data, $kml_file, $project_name, $GPS_period);
mkdir $tmpdir;
recordTourGE($kml_file, $ge_path_vid, $subtitle_length, $vid_out_framerate, $tmpdir, $debug);
generateOverlays();

if($stabilize_video){
    convertFramerate($video_file,$tmp_vid_file,$vid_out_framerate);
    stabilizeVideo($tmp_vid_file,$tmp_vid_file2,$vid_out_framerate);
    meltVideo($tmp_vid_file2,$vid_out_framerate,$subtitle_length,$vid_out_file, $ge_path_vid);
    unlink($tmp_vid_file) or die $!;
    unlink($tmp_vid_file2) or die $!;
}elsif($orig_vid_framerate != $vid_out_framerate){
    convertFramerate($video_file,$tmp_vid_file,$vid_out_framerate);
    meltVideo($tmp_vid_file,$vid_out_framerate,$subtitle_length,$vid_out_file, $ge_path_vid);
    unlink($tmp_vid_file) or die $!;
}else{
    meltVideo($video_file,$vid_out_framerate,$subtitle_length,$vid_out_file, $ge_path_vid);
}
...
sub meltVideo($$$$$){
    ...
    (my $vid_in_file, my $framerate, my $length, my $vid_out_file, my $path_vid_file) = @_;
    ...
    my $command = "melt -track $vid_in_file in=0 out=$num_frames -track $tmpdir/contour_img-%d.png ttl=$frames_per_image in=0 out=$num_frames -track $path_vid_file in=0 out=$num_frames -transition composite: a_track=0 b_track=1 geometry=0%,0%:100%x100%:$base_image_transparency -transition composite: a_track=0 b_track=2 geometry=70%,60%:40%,40%:100 -consumer avformat:$vid_out_file vcodec=$vid_out_codec vpre=$vid_out_quality r=$framerate acodec=$audio_out_codec ab=$audio_out_bitrate";
