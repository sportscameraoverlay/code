...
my $tmp_dir = "/tmp"; #Should be overridden by whoever calls recordTourGE...
my $debug; #Should be overridden by whoever calls recordTourGE...

#my $xvfb_cmd = '/usr/bin/xvfb-run -n ' . $display . ' --server-args="-screen 0 ' . $xvfb_res . 'x24" ' . $ge_bin . ' ' . $kml_file;
#my @record_cmd = ('/usr/bin/recordmydesktop', '--display', ":$display", '--full-shots', '--overwrite', '--no-sound', '--fps', "$vid_out_framerate", '-o', "$output_file",);
#my $xvfb_cmd = '/usr/bin/vncserver :' . $display . ' -geometry ' . $xvfb_res . ' -localhost';

#Keep track of all PIDs so we can kill them if we need to
my $record_pid;
my $xvfb_pid;
my $ge_pid;
...
###############################################################################
sub recordTourGE($$$$$$){
    (my $kml_file, my $output_file, my $record_length, my $rec_framerate, $tmp_dir, $debug) = @_;
...
#when called with 1 means start recording
#0 means stop recording
###############################################################################
sub recordControl($$$){
