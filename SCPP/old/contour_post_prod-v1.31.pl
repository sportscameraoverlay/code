...
# 1.3.1 PJ 28/02/13 Started to fix bugs regarding filling of missing times/dates (not finished!)
#
###############################################################################
...
my $shakiness = 8; #How shakey the video is - Min=1, Max=10
...
sub checkGPSData(){
    ...
    foreach my $epoch_time (sort keys %GPS_data){
        ...
        if($epoch_time != $next_time){
            ...
            print "Missing time value @ epoch: $next_time\n";
        }
...
sub readSubs(){
    ...
    while ($line = <FILE>) {
        ...
        if($gps_checksum eq -1) {
            ...
            #This checks if the time is missing or invalid (as the first test above should grab all valid times)
            }elsif($line =~ /\$GPRMC,.*/){
                $GPRMC_num++;
                #Record any missing times in the missing dates hash as the dates will also be missing
                #$missing_times{ $GPRMC_num } = -1;
                $missing_dates{ $line_num } = -1;
            }
            #We also need to check that the GPGGA data has a epoch time created. This assumes the GPRMC data preceeds the GPGGA data!
            #If there is no valid time I don't really care as the line will e useless anyway
            if ($line =~ /\$GPGGA,(\d+(\.\d+)?),/) {
                my $time = $1;
    ...
    #Fill in the missing dates/times
    if(%missing_dates) {
        ...
        if( ($largest_GPS_time[1] > 230000) and ($smallest_GPS_time[1] < 10000) ) {
            ...
            foreach my $line_num (keys %missing_dates){
                #First lets calculate the date
                my $time = $missing_dates{ $line_num };
                die "Need to fix missing times" if($time == -1);
    ...
    #TODO This should be included above!!
    if(%missing_times) {
