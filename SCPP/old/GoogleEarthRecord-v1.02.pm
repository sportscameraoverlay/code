...
sub recordTourGE($$$$){
    ...
    #Lets wrap all of this code in a eval block so we can do some exception handling...
    try{
        ...
        warn "\nCaught error in GoogleEarthRecord module: $_";
        quitAll("C");
    }
}
...
sub quitAll($){
    ...
    #Kill the progress pid if it is still running
    if(kill 0 => $progress_pid){
        print "Progress PID still running!\n" if($debug);
...
sub recordGEProgress($){
    ...
    $progress_pid = fork;
    defined $progress_pid or die "fork failed: $!";
    if ($progress_pid == 0) { # child
    ...
    print "Progress update PID: $progress_pid\n" if($debug > 1);
    return 1;
}
###############################################################################
