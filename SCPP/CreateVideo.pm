...
sub meltVideo($$$$){
    ...
    my $command;
    #Command if we are adding the path video
    if(@track_pos){
        my @track_size = ($track_pos[2] - $track_pos[0], $track_pos[3] - $track_pos[1]);
        $command = "melt -track \'$vid_in_file\' in=0 out=$num_frames " .
        "-track $path_vid_file in=0 out=$num_frames " .
        "-track $tmp_dir/contour_img-%d.png ttl=$frames_per_image in=0 out=$num_frames " .
        "-transition composite: a_track=0 b_track=1 geometry=$track_pos[0],$track_pos[1]:$track_size[0],$track_size[1]:100 " .
        "-transition composite: a_track=0 b_track=2 geometry=0%,0%:100%,100%:$base_image_transparency " .
        "-consumer avformat:$vid_out_file vcodec=$vid_out_codec vpre=$vid_out_quality r=$vid_out_framerate acodec=$audio_out_codec ab=$audio_out_bitrate";
    }
    #Otherwise if no track vid
    else{
        $command = "melt -track \'$vid_in_file\' in=0 out=$num_frames " .
        "-track $tmp_dir/contour_img-%d.png ttl=$frames_per_image in=0 out=$num_frames " .
        "-transition composite: a_track=0 b_track=1 geometry=0%,0%:100%,100%:$base_image_transparency " .
        "-consumer avformat:$vid_out_file vcodec=$vid_out_codec vpre=$vid_out_quality r=$vid_out_framerate acodec=$audio_out_codec ab=$audio_out_bitrate";
    }
...
sub convertFramerate($$) {
    ...
    my $command;
    if($input_vid_rotation){
        $command = "melt \'$in_filename\' -filter affine transition.fix_rotate_x=$input_vid_rotation -consumer avformat:$out_filename vcodec=$vid_out_codec vpre=$vid_out_quality r=$vid_out_framerate";
    }
    #If we dont need to rotate don't slow things down by applying the filter... (not sure if it does anyway but still)
    else{
        $command = "melt \'$in_filename\' -consumer avformat:$out_filename vcodec=$vid_out_codec vpre=$vid_out_quality r=$vid_out_framerate";
    }
